...
public class WarriorR extends GamePiece {
	
	public WarriorR(int row, int col) {
		super (row, col, Player.LAVA, 20, 4, 4);
		name = "Lava Warrior";
		
	}
	public WarriorR (Position pos) {
		this (pos.r, pos.c);
	}
	...
	public void drawPiece(Graphics2D g2, int x, int y, int width, int height)
		...
		if (level == 0){
			BufferedImage image = ImageIO.read(getClass().getResource("art/warriorR.png"));
			g2.drawImage(image, x, y, null);
		}else{
			BufferedImage image = ImageIO.read(getClass().getResource("art/warriorR2.png"));
			g2.drawImage(image, x, y, null);
		}
        if (selected)
		{
	...
	protected boolean validAttack(int dr, int dc, GameSquares squares) {
		if ((Math.abs(dr) == 1 && Math.abs(dc) == 1) ||
			...
			(Math.abs(dr) == 0 && Math.abs(dc) == 1))
		{
			if (squares.getSquare(pos.r + dr, pos.c + dc).getPiece().getPlayer() == Player.SNOW || squares.getSquare(pos.r + dr, pos.c + dc).getPiece().pieceID == 6){
				return true;
			}
		}
		return false;
