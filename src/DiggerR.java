...
public class DiggerR extends GamePiece {
	
	public DiggerR(int row, int col) {
		super (row, col, Player.LAVA, 15, 2 ,3);
		name = "Lava Miner";
	}
	
	public DiggerR (Position pos) {
		this (pos.r, pos.c);
	}
	...
	public void drawPiece(Graphics2D g2, int x, int y, int width, int height)
		...
		if (level == 0){
			BufferedImage image = ImageIO.read(getClass().getResource("art/diggerR.png"));
			g2.drawImage(image, x, y, null);
		}else{
			BufferedImage image = ImageIO.read(getClass().getResource("art/diggerR2.png"));
			g2.drawImage(image, x, y, null);
		}
	...
	protected boolean validAttack(int dr, int dc, GameSquares squares) {
		if ((Math.abs(dr) == 1 && Math.abs(dc) == 1) ||
			...
			(Math.abs(dr) == 0 && Math.abs(dc) == 1))
		{
			if ((squares.getSquare(pos.r + dr, pos.c + dc).getPiece().getPlayer() == Player.SNOW) ||
